...
export async function getArticle(search: string): Promise<any> {
  if (typeof window !== "undefined") {
    var config = {
      method: "get",
      url: `${process.env.NEXT_PUBLIC_API_URL}/articles/getArticles?search=${search}`,
      headers: {
        Authorization:
    ...
    const response = await axios(config).catch(async function (error) {
      ...
      if (error.response.data.errors === undefined) {
        if (error.response.data.message === "401 jwt expired") {
          ...
          return await getArticle(search);
        }
